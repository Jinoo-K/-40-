...
university_list = df_from_excel["학교이름"].to_list()
...
for i in range(len(university_list)) :
    if position_x_list[i] != 0:
        marker = folium.Marker([position_y_list[i], position_x_list[i]], popup=university_list[i], icon=folium.Icon(color="blue"))
        marker.add_to(map)
